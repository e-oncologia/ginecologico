...
$(document).ready(function() {
	...
	/*Narcar opción*/
	$('.single-multi .respuesta').click(function(event) {
		event.stopPropagation();
		var entorno_actividad = $(this).closest('.grupo-respuestas');
		...
		if ($(entorno_check).hasClass('marcada')) return false;
			...
			var marcados=false;
			$('.grupo-respuestas').each(function(){
				if($(this).find('.respuesta.marcada').length){
					marcados=true;
				}
	...
	/*Comprobar actividad*/
	$('.single-multi .btn-actividad').click(function(event) {
		event.stopPropagation();
		...
		$(entorno_actividad).find('.grupo-respuestas').each(function(){
			if($(this).find('.respuesta.correcta .icono-check').hasClass('marcada')){
				correcto = true;
			}
			else{
				correcto = false;
		...
		$(entorno_actividad).find('.pregunta-global, .pregunta-actividad, .comprobar').hide();
		//subirTop(0);
		activaSiguiente();
