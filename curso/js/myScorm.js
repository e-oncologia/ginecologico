...
//por si acaso la variable está inicializada en el parent, para preservarla entre páginas *jrb
if (parent.initialized == "undefined") {
	var initialized = false;
} else {
	var initialized = parent.initialized;
}
...
function SCOInitialize() {
    var result = doLMSInitialize();
    var tmpScoLessonStatus = doLMSGetValue("cmi.core.lesson_status");
    if (tmpScoLessonStatus == "not attempted") {
        doLMSSetValue("cmi.core.lesson_status", "incomplete");
        doLMSCommit();
    }
    seguimiento = (parseInt(doLMSGetValue("cmi.core.lesson_location")));
    if (isNaN(seguimiento)) {
        doLMSSetValue("cmi.core.lesson_location", "0");
        doLMSCommit();
        seguimiento =0;
    }
    iniciarcurso();
...
function actualizaSeguimientoScorm() {
    doLMSSetValue("cmi.core.lesson_location", seguimiento);
    doLMSCommit();
}

function actualizaProgreso(score) {
    doLMSSetValue("cmi.core.score.raw", score);
    doLMSCommit();
}

function actualizaFin(estatus) {
    doLMSSetValue("cmi.core.lesson_status", estatus);
    doLMSCommit();
}